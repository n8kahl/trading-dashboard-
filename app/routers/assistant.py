...
async def assistant_exec(payload: Dict[str, Any]):
    ...
    # Minimal, predictable shapes (safe defaults)
    ...
    if op == "market.quote":
        ...
        if not sym:
            raise HTTPException(status_code=400, detail="args.symbol required")
        return {"ok": True, "quote": {"symbol": sym, "bid": None, "ask": None, "last": None}}

    if op == "options.chain":
        sym = (args.get("symbol") or "").upper()
        if not sym:
            raise HTTPException(status_code=400, detail="args.symbol required")
        return {"ok": True, "chain": {"symbol": sym, "expiry": args.get("expiry"), "items": []}}
    ...
    if op == "coach.guidance":
        ...
        return {"ok": True, "guidance": {
            ...
            "rationale": ["Boot-stub; wire providers/LLM next"],
