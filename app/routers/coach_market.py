...
from app.services.market_overview import enrich_with_events, market_open_snapshot
    ...
    try:
        ...
        if req.events_source == "polygon":
            ...
            for e in poly.get("earnings", []):
                ...
                note = e.get("note", "")
                merged.append(
                    ("Earnings " + sym + (" (" + when + ")" if when else "") + (" " + note if note else "")).strip()
                )
        ...
        if req.horizon == "week":
            from app.services.earnings_provider_finnhub import (
                fetch_earnings_ahead_finnhub,
            )
            ...
            if "note" not in week or "events" not in week:
                week = {
                    "note": week.get("note") if isinstance(week, dict) else None,
                    "events": week.get("events") if isinstance(week, dict) else [],
                }
            if "note" not in earn or "earnings" not in earn:
                earn = {
                    "note": earn.get("note") if isinstance(earn, dict) else None,
