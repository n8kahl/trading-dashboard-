...
async def _fetch_bars(symbol: str, minutes_back: int = 60) -> List[Dict[str,Any]]:
    try:
        md = await get_timesales_1min(symbol, minutes_back=minutes_back)
        return md.get("bars") or []
    except Exception:
        return []
...
=======
    ...
    if not _CACHE:
        return
    symbols = sorted({a["symbol"] for a in _CACHE.values()})
