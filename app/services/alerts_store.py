import os, psycopg2, json, datetime as dt
...
def list_by_symbol(symbol: str) -> List[Dict[str,Any]]:
    ...
    with _conn() as c, c.cursor() as cur:
        ...
        ORDER BY id;
        """, (symbol.upper(),))
        return [{"id":r[0],"symbol":r[1],"timeframe":r[2],"condition":r[3],
                 "active":r[4],
                 "expires_at": r[5].isoformat() if r[5] else None,
                 "last_triggered_at": r[6].isoformat() if r[6] else None,
...
def add_trigger(alert_id: int, symbol: str, payload: Dict[str,Any]):
    if not DB_URL: return
    with _conn() as c, c.cursor() as cur:
        cur.execute("INSERT INTO alert_triggers(alert_id,symbol,payload) VALUES (%s,%s,%s);",
...
def recent_triggers(limit: int = 50) -> List[Dict[str,Any]]:
    ...
    with _conn() as c, c.cursor() as cur:
        ...
        for r in rows:
            payload = r[4] or {}
            out.append({"id":r[0], "alert_id":r[1], "symbol":r[2],
