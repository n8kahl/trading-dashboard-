...
from app.services.indicators import ema, atr, vwap, sma, rsi, macd
...
def sig_divergence_catch(close, rsi14, lookback: int = 15):
    ...
    for i in range(lookback+1, len(close)):
        ...
        if p2_idx <= p1_idx:
            continue
        p1 = window_p[p1_idx]; p2 = window_p[p2_idx]
        r1 = window_r[p1_idx]; r2 = window_r[p2_idx]
...
def run_strategy(bars: Dict[str, List[float]], strategy: Literal["trend_pullback","vwap_reclaim","range_break_retest","divergence_catch"], cfg: SimConfig) -> Dict[str, Any]:
