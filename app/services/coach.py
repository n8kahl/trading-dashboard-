...
def explain(e: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not liq_ok:
        lines.append(
            f"• Liquidity: ❌ ({_msg(e, 'liquidity')}) — need RVOL≥{POLICY['equities']['rvol_min']}, "
    ...
    if not entry_ok:
        ...
        if reasons:
            lines.append(
                f"• Entry timing: ❌ ({', '.join(reasons)}) — require retest+1-bar hold "
    ...
    if pr_win is not None and expR is not None:
        lines.append(
            f"• Thresholds: need Pr(win)≥{POLICY['decision']['min_pr_win']:.2f} "
    ...
    if take:
        ...
        if plan:
            lines.append(
                f"Plan: entry {plan.get('entry')}, stop {plan.get('stop')}, TP1 {plan.get('tp1')} "
