...
def evaluate_components(cx: Dict, w: Dict) -> List[Component]:
    ...
    comps.append(
        Component(
            "ema_stack", w["ema_stack"] if ema9_gt else -w["ema_stack"], "EMA9 > EMA20" if ema9_gt else "EMA9 < EMA20"
        )
    )
    ...
    comps.append(
        Component(
            "volume_confluence",
            w["volume_confluence"] if vol_ok else -int(w["volume_confluence"] * 0.8),
    ...
    if cx.get("spread_pct") is not None:
        ...
        if s <= 6:
            pts, ex = int(w["liquidity"] * 0.5), f"Spread {s:.1f}% (tight)"
        elif s <= 8:
            pts, ex = int(w["liquidity"] * 0.2), f"Spread {s:.1f}% (acceptable)"
        elif s <= 10:
            pts, ex = -int(w["liquidity"] * 0.6), f"Spread {s:.1f}% (wide)"
        else:
            pts, ex = -w["liquidity"], f"Spread {s:.1f}% (too wide)"
        comps.append(Component("liquidity", pts, ex))
    if cx.get("iv_percentile") is not None:
        ivp = int(cx["iv_percentile"])
        if 20 <= ivp <= 70:
            comps.append(Component("vol_context", int(w["vol_context"] * 0.6), f"IVP {ivp} (mid)"))
        elif ivp < 20:
            comps.append(Component("vol_context", int(w["vol_context"] * 0.3), f"IVP {ivp} (low)"))
        else:
            comps.append(Component("vol_context", -w["vol_context"], f"IVP {ivp} (high)"))
...
def compute_confluence_score(context: Dict, strategy_id: str) -> Dict:
    ...
    return {
        ...
        "inputs": context,
        "score": {"total": total, "band": band(total), "components": [c.__dict__ for c in comps]},
    }
