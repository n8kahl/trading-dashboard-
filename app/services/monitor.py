...
async def check_trades_once(db: Session):
    ...
    for t in open_trades:
        ...
        stop = plan.get("stop")
        tps  = plan.get("tp", [])
        hit_tp = next((tp for tp in tps if px >= tp), None) if t.side in ("CALL","LONG") else next((tp for tp in tps if px <= tp), None)
        hit_stop = (px <= stop) if stop is not None and t.side in ("CALL","LONG") else (px >= stop) if stop is not None else False
