...
from app.models import PaperFill, PaperPosition, PaperTrade
...
def _daily_loss_r(db: Session) -> float:
    ...
    today = date.today()
    q = select(func.coalesce(func.sum(PaperTrade.outcome_r), 0.0)).where(func.date(PaperTrade.close_time) == today)
    ...
    if intent.side == "buy":
        ...
        db.add(
            PaperFill(
                ...
                qty=intent.qty,
                time=now,
                liquidity_bucket=intent.session,
                spread_pct=bps / 100.0,
            )
        )
        db.commit()
        ...
        return {
            ...
            "remaining_qty": pos.qty if pos.qty > 0 else 0,
        }
...
def positions(db: Session) -> List[Dict[str, Any]]:
    ...
    return [
        {"symbol": r.symbol, "qty": r.qty, "avg_px": r.avg_px, "opened_at": r.opened_at.isoformat() + "Z"} for r in rows
    ]
...
def pnl_for_date(db: Session, qdate: Optional[str]) -> Dict[str, Any]:
    ...
    rows = (
        db.execute(select(PaperTrade).where(PaperTrade.close_time.isnot(None), func.date(PaperTrade.close_time) == day))
        .scalars()
        .all()
    )
