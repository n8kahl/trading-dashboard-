...
def plan_vwap_bounce(c: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if price is None:
        return {"entry_hint": None, "stop_loss": None, "take_profits": [], "sizing_hint": "small", "notes": "Price unavailable"}
...
def plan_opening_range_breakout(c: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if price is None or orh is None or orl is None:
        return {"entry_hint": None, "stop_loss": None, "take_profits": [], "sizing_hint": "small", "notes": "Opening range not available; plan disabled"}
...
def plan_power_hour_spx(c: Dict[str, Any]) -> Dict[str, Any]:
    price = _safe(c.get("price")); vwap = _safe(c.get("vwap"))
    if not c.get("is_power_hour", False) or price is None:
        return {"entry_hint": None, "stop_loss": None, "take_profits": [], "sizing_hint": "small", "notes": "Not in power hour or price unavailable"}
