...
_headers_tradier = (
    {"Authorization": f"Bearer {TRADIER_ACCESS_TOKEN}", "Accept": "application/json"} if TRADIER_ACCESS_TOKEN else {}
)
...
async def _aget_json(
    url: str, headers: Dict[str, str] = None, params: Dict[str, Any] = None, method: str = "GET", data: Any = None
) -> Tuple[int, Any]:
...
async def get_last_price(symbol: str, client: Optional[TradierClient] = None) -> Optional[float]:
    ...
    if POLYGON_KEY:
        ...
        code, js = await _aget_json(
            f"https://api.polygon.io/v2/aggs/ticker/{symbol}/range/1/day/2020-01-01/{today}",
...
async def get_tradier_chain_by_exp(symbol: str, expiration: str, opt_type: str) -> List[Dict[str, Any]]:
    ...
    out = []
    for o in opts:
        if (o.get("option_type") or "").lower() != side:
            continue
        bid = o.get("bid")
        ask = o.get("ask")
        spread_pct = None
