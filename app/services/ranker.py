...
def get_ranked_picks(mode: str = "sandbox") -> Dict[str, Any]:
    ...
    picks: List[Dict[str, Any]] = [
        {"symbol":"AAPL250912C00240000","expiration":"2025-09-12","strike":240.0,"option_type":"call",
         "delta":0.49,"bid":2.71,"ask":2.75,"mark":2.73,"spread_pct":0.0145,"open_interest":39019,"volume":50900,"dte":4},
        {"symbol":"AAPL250912C00242500","expiration":"2025-09-12","strike":242.5,"option_type":"call",
         "delta":0.35,"bid":1.64,"ask":1.68,"mark":1.66,"spread_pct":0.0238,"open_interest":8356,"volume":28426,"dte":4},
    ]
    for p in picks:
        p["score"] = _score_contract(p)
    return {
        "ok": True, "env": mode, "note": f"{mode} ranked (heuristic)", 
        "count_considered": len(picks), "picks": sorted(picks, key=lambda x: x["score"], reverse=True)
    }
