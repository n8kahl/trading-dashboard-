...
def score_vwap_bounce(c: Dict[str, Any]) -> Dict[str, int | float]:
    ...
    if vwap is not None and price is not None:
        if price > vwap:
            pts = min(15, 5 + 3 * bars)
            comp["vwap_posture"] = pts; score += pts
        else:
            comp["vwap_posture"] = -10; score -= 10
    if c.get("ema9_gt_ema20") is True: comp["ema_stack"] = 12; score += 12
    elif c.get("ema9_gt_ema20") is False: comp["ema_stack"] = -8; score -= 8
    div = (c.get("divergence_5m") or "").lower()
    if "bullish_confirmed" in div: comp["divergence"] = 10; score += 10
    elif "bearish_confirmed" in div: comp["divergence"] = -10; score -= 10
    else: comp["divergence"] = 0
    sp = _safe_num(c.get("spread_pct"))
    if sp is not None:
        if sp <= 5: comp["liquidity"] = 10; score += 10
        elif sp <= 8: comp["liquidity"] = 4; score += 4
        else: comp["liquidity"] = -8; score -= 8
    ivp = _safe_num(c.get("iv_percentile"))
    if ivp is not None:
        if 30 <= ivp <= 70: comp["vol_context"] = 4; score += 4
        elif ivp > 80: comp["vol_context"] = -4; score -= 4
        else: comp["vol_context"] = 0
    if c.get("is_macro_window") is True: comp["macro"] = -5; score -= 5
    return {"score": max(0, min(100, score)), "components": comp}

def score_ema_crossover(c: Dict[str, Any]) -> Dict[str, int | float]:
    score = 0; comp = {}
    if c.get("ema9_gt_ema20") is True: comp["ema_stack"] = 25; score += 25
    elif c.get("ema9_gt_ema20") is False: comp["ema_stack"] = -15; score -= 15
    rsi_hint = (c.get("divergence_5m") or "").lower()
    if "bullish" in rsi_hint: comp["momentum_hint"] = 5; score += 5
    vwap = c.get("vwap"); price = c.get("price")
    if vwap is not None and price is not None:
...
def score_opening_range_breakout(c: Dict[str, Any]) -> Dict[str, int | float]:
    ...
    if orh is None or orl is None or price is None:
        ...
        return {"score": max(0, min(100, score)), "components": comp}
    if price > orh:
        comp["breakout"] = 25; score += 25
    elif price < orl:
        comp["breakout"] = -20; score -= 20
    else:
        comp["breakout"] = -5; score -= 5
    rv5 = _safe_num(c.get("rel_volume_5"))
    if rv5 is not None:
        if rv5 >= 1.5: comp["volume_conf"] = 15; score += 15
        elif rv5 >= 1.1: comp["volume_conf"] = 7; score += 7
        else: comp["volume_conf"] = -5; score -= 5
    if c.get("ema9_gt_ema20") is True: comp["ema_stack"] = 10; score += 10
    vwap = _safe_num(c.get("vwap"))
    if vwap is not None and price is not None:
        comp["vwap_posture"] = 8 if price > vwap else -6; score += comp["vwap_posture"]
    return {"score": max(0, min(100, score)), "components": comp}
...
def score_power_hour_spx(c: Dict[str, Any]) -> Dict[str, int | float]:
    ...
    if not c.get("is_power_hour", False) or sym not in {"SPX","SPY","ES","MES"}:
        comp["window_check"] = -10; score -= 10
        return {"score": max(0, min(100, score)), "components": comp}
    price = _safe_num(c.get("price")); vwap = _safe_num(c.get("vwap"))
    if price is not None and vwap is not None:
        comp["vwap_posture"] = 15 if price > vwap else -10; score += comp["vwap_posture"]
    if c.get("ema9_gt_ema20") is True: comp["ema_stack"] = 15; score += 15
    rv5 = _safe_num(c.get("rel_volume_5"))
    if rv5 is not None:
        comp["rel_volume_5"] = 10 if rv5 >= 1.2 else 0; score += comp["rel_volume_5"]
    p = price; pdh = _safe_num(c.get("prev_day_high")); pdl = _safe_num(c.get("prev_day_low"))
    if p is not None and pdh is not None and pdl is not None and pdh > pdl:
        pos = (p - pdl) / (pdh - pdl)
        adj = 10 if pos >= 0.6 else (-5 if pos <= 0.4 else 0)
        comp["range_pos"] = adj; score += adj
    return {"score": max(0, min(100, score)), "components": comp}
