...
from .ta import ema, rsi, atr, vwap
...
def strategy_scalp_momentum(bars: List[dict]) -> Dict[str, Any]:
    ...
    score = 0
    rationale = []
    if e9 and e20 and e9 > e20: score += 25; rationale.append("EMA9 > EMA20 (uptrend)")
    if r and 52 <= r <= 70:     score += 20; rationale.append(f"RSI healthy {r:.1f}")
    if closes[-1] > (e9 or closes[-1]): score += 15; rationale.append("Price above EMA9 (momentum intact)")
...
def strategy_breakout_retest(bars: List[dict]) -> Dict[str, Any]:
    ...
    last = closes[-1]
    score = 0; rationale=[]
    if e50 and last > e50: score += 20; rationale.append("Above EMA50 (trend filter)")
    if last >= recent_high * 0.995: score += 25; rationale.append("Near 20-bar high (breakout zone)")
    if bars[-1]["l"] <= recent_high and last > recent_high: score += 15; rationale.append("Retest and reclaim of breakout")
    return {"id":"breakout_retest","name":"Breakout Retest","score":score,"side":"CALL" if last>=recent_high else "FLAT","notes":rationale,"level":recent_high}
...
def strategy_reversal_divergence(bars: List[dict]) -> Dict[str, Any]:
    ...
    if len(closes) >= 20:
        ...
        if p2 < p1 and r1 is not None and r2 is not None and r2 > r1:
            score += 30; rationale.append("Bullish divergence (price ↓, RSI ↑)")
    if r14[-1] is not None and r14[-1] < 35:
        score += 20; rationale.append(f"RSI oversold {r14[-1]:.1f}")
    return {"id":"rev_div","name":"Reversal Divergence","score":score,"side":"CALL" if score>=30 else "FLAT","notes":rationale}
...
def strategy_vwap_fade(bars: List[dict]) -> Dict[str, Any]:
    ...
    if ext > 0.02:
        score += 25; rationale.append(f"Price {ext*100:.1f}% above VWAP (overextension)")
    return {"id":"vwap_fade","name":"VWAP Fade","score":score,"side":"PUT" if score>=25 else "FLAT","notes":rationale}
